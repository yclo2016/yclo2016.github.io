...
### 步驟 2：用「對話」而不是「規格書」來開始

我一開始只問了一句「這個 repo 是什麼？」Claude 發現 repo 是空的，就解釋了 `yclo2016.github.io` 這種名稱代表 GitHub Pages 個人網站。接著我問：

```text
我可以在這裡放一個個人的部落格，來放一些文章跟技術文件嗎？
```

Claude 比較了幾種做法，推薦 Jekyll（GitHub Pages 原生支援，文章用 Markdown 寫）。我又補充了用途：

```text
我想要用這個東西來做研究與教學：譬如我對某些主題有興趣，
可能會請你幫我做研究，並寫一些手把手的教學文章給我看，
也許也可以分享給其他人看。這樣的話，你覺得用什麼樣的風格會比較好？
介面以繁體中文為主，但專有名詞需要中英對照。
```

Claude 根據這個用途推薦了 **Chirpy** 主題，因為它有文章目錄、程式碼高亮、提示框、流程圖和全站搜尋，很適合教學文。

> **原來可以這麼想**
>
> 你不需要一開始就知道該用什麼技術。**先說清楚你想達成的目標和用途**，再讓 Claude 提出選項和推薦理由，由你做決定。這比你自己上網查「最好的部落格框架」快得多，而且答案是針對你的需求。
{: .prompt-tip }

### 步驟 3：讓 Claude 動手，並且邊做邊修正方向

確定標題（AI Research Notes）和顯示名稱（YC）後，我請 Claude 開始建站並寫第一篇文章。在它工作的過程中，我陸續補了幾句要求，它都會即時收到並調整：

```text
我希望這個文章能夠提供具體、一步步的操作指導，並使用真正有意義的例子，
而不是那種為了教學而創造的無病呻吟的例子。
```

```text
或是～原來還可以這麼用啊？原來還可以這麼思考啊？的感覺。
```

這篇文章裡每個「原來可以這麼想」的提示框，就是這兩句話的成果。

> **原來可以這麼想**
>
> Claude 工作時，你**不用等它做完才能說話**。發現方向不對就直接補一句，它會在下一步把你的意見納入考量。就像跟同事協作，而不是填好表單送出後等結果。
{: .prompt-tip }

### 步驟 4：把規則寫進 `CLAUDE.md`，讓以後的每個工作階段都記得
...
> **原來可以這麼想**
>
> 注意這個任務用的例子是**本站自己的檔案**。與其讓 AI 用一個虛構的範例解釋 GitHub Actions，不如請它解釋「你手上正在用的那個設定檔」。你會同時學到概念，也真的看懂了自己的網站。
>
...
| 要先搞懂技術才能開始 | 先說清楚目標，讓 AI 提選項、你做決定 |
